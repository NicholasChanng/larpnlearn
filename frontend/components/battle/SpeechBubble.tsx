...
export function SpeechBubble({ text, from = "right" }: { text: string; from?: "left" | "right" }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10, scale: 0.95 }}
      animate={{ opacity: 1, y: 0, scale: 1 }}
      className={`relative max-w-xl rounded-2xl border-4 border-black bg-white px-5 py-4 text-slate-900 shadow-2xl ${
